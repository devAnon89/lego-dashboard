...
const commands = {
  status: () => {
    ...
    const buys = Object.values(analysis).filter(
      (a) => a.action === 'BUY'
    ).length;
    const holds = Object.values(analysis).filter(
      (a) => a.action === 'HOLD'
    ).length;
    const sells = Object.values(analysis).filter(
      (a) => a.action === 'SELL'
    ).length;
    ...
    console.log(
      `📈 Gain: €${gain.toFixed(2)} (${gain >= 0 ? '+' : ''}${gainPct}%)`
    );
    console.log('─'.repeat(50));
  ...
  analyze: (setId) => {
    ...
    console.log(
      `Retail: €${set.retail} | Paid: €${set.paid} | Value: €${set.value}`
    );
    console.log(`Qty: ${set.qty_new || 0} new, ${set.qty_used || 0} used`);
    console.log(
      `P&L: €${gain.toFixed(2)} (${set.growth_pct?.toFixed(1) || 0}%)`
    );
    ...
    if (aiAnalysis) {
      ...
      console.log(
        `Recommendation: ${aiAnalysis.action} (${aiAnalysis.confidence})`
      );
  ...
  themes: () => {
    ...
    const sorted = Object.entries(themes).sort(
